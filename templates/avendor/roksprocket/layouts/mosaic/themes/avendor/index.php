...
<div class="sprocket-mosaic" data-mosaic="<?php echo $parameters->get('module_id'); ?>">
	...
	<div class="sprocket-mosaic-header">
		...
		<div class="sprocket-mosaic-filter mt10 mb30">
			<ul>
				<?php foreach($tagging as $key => $filter): ?>
				<li class="btn btn-default btn-alt btn-xs <?php echo $key;?><?php echo $key == 'all' ? ' btn-primary' : '';?>" data-mosaic-filterby="<?php echo $key; ?>"><?php echo $filter; ?></li>
				<?php endforeach; ?>
			</ul>
		</div>
		<?php endif; ?>
		...
		<div class="sprocket-mosaic-order mt10 mb30">
			<ul>
				<?php foreach($ordering as $i => $order): ?>
				<li class="btn btn-default btn-alt btn-xs <?php echo !$i && $order != 'random' ? 'btn-primary' : ''; ?>" data-mosaic-orderby="<?php echo $order; ?>"><?php rc_e('ROKSPROCKET_MOSAIC_ORDERING_' . strtoupper($order)); ?></li>
				<?php endforeach; ?>
			</ul>
		</div>
		<?php endif; ?>
	...
	<ul class="sprocket-mosaic-container sprocket-mosaic-columns-<?php echo $parameters->get('mosaic_columns');?> portfolio-classic" data-mosaic-items>
		<?php
			$index = 0;
	...
	<?php if ($pages > 1): ?>
	<div class="sprocket-mosaic-loadmore btn btn-default btn-alt" data-mosaic-loadmore>
